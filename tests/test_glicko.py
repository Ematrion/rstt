...
def test_rate_mu():
    assert glicko.rate(test_player, opponents, scores).mu == pytest.approx(updated_player_rating.mu, 0.1)
    
def test_rate_sigmae():
    assert glicko.rate(test_player, opponents, scores).sigma == pytest.approx(updated_player_rating.sigma, 0.1)
